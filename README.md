# 一个通过了全部样本内检验、却死在样本外的宏观板块信号

**美股板块相对配置模型 · 宏观流动性、股权久期,与 QE 时代的边界**
...
我做了一个宏观驱动的板块倾斜模型,用严格的诊断砍掉了三个维度里的两个,
剩下的那个看起来相当不错:20日 IC **0.189**、Newey-West t 值 **3.14**
(滞后拉到一整年仍不塌)、跨 8 个板块对 **8/8** 夏普为正。

然后我把它推到 **2000–2007**——一段从未参与任何设计决策的历史:

| | 2008–2019(设计期) | 2000–2007(样本外) |
|---|---|---|
| IC 中位数 (20日) | **+0.143** | **+0.026** |
| IC 为正的板块对 | 6 / 8 | 5 / 8 |
| 净夏普中位数 | **+0.346** | **−0.171** |
| **夏普为正的板块对** | **8 / 8** | **0 / 8** |

**模型没有通过。** 不是勉强通过或轻微衰减——八个板块对**全部翻负**。
而且失效有明确的机制,不是一句"统计噪音"能带过的。

![结构性断点](charts/06_regime_break.png)

断点精准落在 2008 年。同一个四信号模型,在 QE 开始之后比之前强 3–5 倍。

**这个项目真正支撑的结论**不是"我找到了一个信号",而是:

> "央行放水 → 长久期资产跑赢"这个关系,**成立的前提是央行资产负债表政策
> 主导资产定价**。这个前提 2008 年起成立,之前不成立。
> 任何时候要用这个模型,第一步都得先问:这个前提今天还在吗?

---

## 一点五、样本内的模型长什么样

（下面几节记录的是**被推翻之前**的完整结果。保留它们不是为了自夸，
而是因为"样本内什么都通过、样本外全线崩溃"这个反差本身就是结论的一部分。）

**模型定义**：仅用宏观流动性维度（5 个信号等权），月频调仓，T+1 收盘执行。
...
| **Regime 判定因果性** | 截断重跑后状态序列逐点相同 |
| **regime 模式下的截断不变性** | 开启 regime 后信号面板同样不能泄漏 |
...
### 3.5 多板块对交叉检验：机制被验证，但样本量没有

8 个板块对全部独立跑一遍，7/8 的 IC 为正、8/8 的夏普为正。更有意思的是**强弱分布**：

```
corr(海外收入差, IC) = +0.85
corr(久期差,     IC) = +0.70
corr(贝塔差,     IC) = -0.09   ← 完全无关
```

三个近似为零的板块对，恰好就是久期差和海外收入差都小的那三个。
**信号出现在理论说它该出现的地方，在该失效的地方失效**——这是机制验证。
它也再次修正了模型的定性：这不是通用的板块轮动信号，而是一个
**久期 + 汇率暴露因子**。

⚠️ **但这 8 个对不是 8 次独立检验。** 信号两两相关中位数 **0.82**、
第一主成分解释 **77%** 的方差，等效独立对数只有约 **1.6** 个
（5 个对共用科技做多头腿）。合并出来的 226 个事件在时间上只聚成 **54 波**。
如果直接拿 226 个事件去跑假设检验，p 值会小得离谱，然后得出一堆假规律——
**这是扩样本时最容易踩的坑**。
...
2000–2007 这段**从未参与任何设计决策**——维度筛选、信号选择、月频、迟滞带，
全部是在 2008 年之后的数据上定的。

5 个宏观信号里有 3 个在 2007 年前不存在，只能用长历史替代：
名义 10 年美债代实际利率、主要货币美元指数代广义美元指数，
**联储总资产直接丢弃**（无合适的长历史替代）。

**让检验成立的关键控制：两段跑的是同一个 4 信号简化模型。**
否则"样本外更差"分不清是模型失效还是信号变少。

### 结果

![逐对样本外对比](charts/07_oos_by_pair.png)

简化模型在 2008–2019 依然有效（IC 0.143、8/8 夏普为正），
**所以失效不是因为丢了联储总资产**——同样四个信号，2008 年前就是没有信息。

### 失效机制

![科网泡沫失效](charts/08_dotcom_failure.png)

| 时期 | 模型平均分差 | 建议超配科技的天数 | 实际科技相对必需消费 |
|---|---|---|---|
| 2000-06 ~ 2001-03 泡沫破裂初期 | **+0.19** | 69% | **-50.3%** |
| 2001-03 ~ 2002-10 联储 6.5%→1% | +0.05 | 54% | **-43.9%** |
| 2002-10 ~ 2003-12 见底回升 | +0.11 | 73% | +54.4% |
| 2004-01 ~ 2006-06 加息周期 | -0.12 | 38% | -9.0% |

**科技相对崩塌的全程，模型三分之二的时间在喊超配科技。**
联储从 6.5% 降到 1%，利率信号把暴跌的贴现率读成了长久期资产的顺风——
而那轮降息恰恰是央行**对这类资产崩塌的反应**。
...
### 这同时把 3.4 节的 regime 结论反转了

我在 regime 检验里的先验是"压力期的降息/扩表是被动救火，信号被污染，应该衰减"。
2008 年后的数据说我错了（联储扩表在 risk-off 下 IC 翻倍），于是整个方案被否决。

**但在 2001–2003，那个直觉完全正确。** 后来之所以被否决，
只是因为 2008 年后的样本被 2009 和 2020 两次"QE 标记底部"主导了。

也就是说：这个模型的核心关系**本身就是随时代变的**。
一个"被证伪的先验"其实是"一个自身也依赖 regime 的先验"——
这比原来的否决结论更能说明模型的不稳定性。
